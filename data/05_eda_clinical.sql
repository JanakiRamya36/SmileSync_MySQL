...
FROM (
	...
	JOIN treatment_info t 
		ON d.did = t.did
    GROUP BY c.cid
) visit_treatment_counts;
  

---  SECTION 3: CLINICAL PATTERNS
-- Frequently reported chief complaints
SELECT 
	NULLIF(TRIM(chief_complaint),'') AS cheif_compliant,
    COUNT(*) AS freq
FROM consultation_info
...
-- Diagnosis burden by age group and gender
WITH age_groups AS (
    SELECT
        p.PID,
        p.gender,
...
-- Treatment rate per doctor
SELECT 
    doc.Name AS doctor_name,
    d.diagnosis,
    COUNT(d.DID) AS total_cases,
    SUM(CASE WHEN t.TID IS NOT NULL THEN 1 ELSE 0 END) AS treated_cases,
    ROUND(SUM(CASE WHEN t.TID IS NOT NULL THEN 1 ELSE 0 END)/COUNT(d.DID)*100,2) AS treatment_rate_pct
FROM diagnosis_info d
JOIN doctor_info doc 
	ON d.DocID = doc.DocID
LEFT JOIN treatment_info t 
	ON d.DID = t.DID
GROUP BY doc.Name, d.diagnosis
ORDER BY treatment_rate_pct DESC;
